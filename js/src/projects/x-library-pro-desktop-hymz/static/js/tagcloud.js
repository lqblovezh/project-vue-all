...
window.tagcloud = (function(win, doc) {
  ...
  function TagCloud(options) {
    ...
    var item = self.box.querySelectorAll('li')
    for (var i = 0; i < item.length; i++) {
      TagCloud._on(item[i], 'mouseover', function() {
        clearInterval(self.up)
        self.active = true
      })
      TagCloud._on(item[i], 'mouseleave', function() {
        banner_scorll()
        self.active = false
      })
    }
  ...
  TagCloud.prototype = {
    ...
    update: function() {
      ...
      for (var j = 0, len = self.items.length; j < len; j++) {
        ...
        self.items[j].y = ry2
        self.items[j].z = rz2
        self.items[j].scale = per > 1.1 ? 1.1 : per
        self.items[j].fontsize = Math.ceil(per * 3) + self.fontsize - 6
        self.items[j].alpha = 1.5 * per - 0.5 //取值范围0.6 ~ 3
        ...
        self.items[j].element.style.transform =
          'scale(' + self.items[j].scale + ')'
      }
    },
    ...
    _getItems: function() {
      ...
      for (var i = 0; i < length; i++) {
        ...
        item.x =
          self.radius * Math.cos(item.angle.theta) * Math.sin(item.angle.phi)
        item.y =
          self.radius * Math.sin(item.angle.theta) * Math.sin(item.angle.phi)
